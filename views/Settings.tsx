...
import { getDepartments, addDepartment, deleteDepartment, CurriculumDepartment, getTracks, addTrack, deleteTrack, AcademicTrack } from '../services/supabaseData';
import { 
...
export const Settings: React.FC<SettingsProps> = ({ role, language }) => {
  ...
  // ============ الأقسام المنهجية ============
  const [departments, setDepartments] = useState<CurriculumDepartment[]>([]);
  ...
  const handleAddDepartment = async () => {
    ...
    const id = await addDepartment(newDeptName.trim());
    if (id) {
      refreshDepartments();
      setNewDeptName('');
  ...
  const [newTrackGradeIds, setNewTrackGradeIds] = useState<string[]>([]);
  const refreshTracks = () => { getTracks().then(setTracks); };
  useEffect(() => { refreshTracks(); }, []);
  const handleAddTrack = async () => {
    if (!newTrackName.trim() || newTrackGradeIds.length === 0) return;
    const id = await addTrack(newTrackName.trim(), newTrackGradeIds);
    if (id) {
      refreshTracks();
      setNewTrackName('');
      setNewTrackGradeIds([]);
        ...
        <div className="max-w-5xl mx-auto p-8 lg:p-12">
                          ...
                          <div className="space-y-8 animate-fadeIn">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                              <div className="space-y-2 md:col-span-2">
                                ...
                                >
                                  {departments.length === 0 && <option value="General">{isRTL ? 'عام' : 'General'}</option>}
          ...
          {activeTab === SettingsTab.GRADE_LEVELS && (
            <div className="flex-1 flex flex-col">
              <div className="mb-8">
                ...
                <p className="text-sm text-gray-500 mt-2">
                  {isRTL ? 'الصفوف والأقسام والمسارات اللي بتستخدمها المدرسة في كل مكان بالنظام' : 'The grades, departments, and tracks your school uses throughout the system'}
                </p>
              </div>
              ...
              <div className="space-y-6">
                ...
                <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    {isAddingGradeLevel ? (
                      <div className="flex items-center gap-2">
                        <button onClick={() => { setIsAddingGradeLevel(false); setNewGradeLevelName(''); }} className="text-sm font-bold px-4 py-2 rounded-xl bg-gray-100 text-gray-600 hover:bg-gray-200 transition-colors">
                          {isRTL ? 'إلغاء' : 'Cancel'}
                        </button>
                        <button onClick={() => { setIsAddingGradeLevel(false); setNewGradeLevelName(''); }} className="text-sm font-bold px-4 py-2 rounded-xl bg-emerald-600 text-white hover:bg-emerald-700 transition-colors">
                          {isRTL ? 'حفظ' : 'Save'}
                        </button>
                      </div>
                    ) : (
                      <button onClick={() => setIsAddingGradeLevel(true)} className="text-sm font-bold px-4 py-2 rounded-xl flex items-center gap-1.5 transition-colors bg-violet-600 text-white hover:bg-violet-700">
                ...
                <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    {isAddDeptModalOpen ? (
                      <div className="flex items-center gap-2">
                        <button onClick={() => { setIsAddDeptModalOpen(false); setNewDeptName(''); }} className="text-sm font-bold px-4 py-2 rounded-xl bg-gray-100 text-gray-600 hover:bg-gray-200 transition-colors">
                          {isRTL ? 'إلغاء' : 'Cancel'}
                        </button>
                        <button onClick={() => { setIsAddDeptModalOpen(false); setNewDeptName(''); }} className="text-sm font-bold px-4 py-2 rounded-xl bg-emerald-600 text-white hover:bg-emerald-700 transition-colors">
                          {isRTL ? 'حفظ' : 'Save'}
                        </button>
                      </div>
                    ) : (
                      <button onClick={() => setIsAddDeptModalOpen(true)} className="text-sm font-bold px-4 py-2 rounded-xl flex items-center gap-1.5 transition-colors bg-violet-600 text-white hover:bg-violet-700">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {departments.map(d => (
                      <span key={d.id} className="flex items-center gap-2 bg-gray-50 border border-gray-100 text-gray-700 px-3 py-1.5 rounded-full text-sm font-bold">
                        {d.name}
                        <button onClick={() => handleDeleteDepartment(d.id)} className="text-gray-300 hover:text-red-500">
                          <X size={13} />
                        </button>
                      </span>
                    ))}
                ...
                <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    {isAddTrackModalOpen ? (
                      <div className="flex items-center gap-2">
                        <button onClick={() => { setIsAddTrackModalOpen(false); setNewTrackName(''); setNewTrackGradeIds([]); }} className="text-sm font-bold px-4 py-2 rounded-xl bg-gray-100 text-gray-600 hover:bg-gray-200 transition-colors">
                          {isRTL ? 'إلغاء' : 'Cancel'}
                        </button>
                        <button onClick={() => { setIsAddTrackModalOpen(false); setNewTrackName(''); setNewTrackGradeIds([]); }} className="text-sm font-bold px-4 py-2 rounded-xl bg-emerald-600 text-white hover:bg-emerald-700 transition-colors">
                          {isRTL ? 'حفظ' : 'Save'}
                        </button>
                      </div>
                    ) : (
                      <button onClick={() => setIsAddTrackModalOpen(true)} className="text-sm font-bold px-4 py-2 rounded-xl flex items-center gap-1.5 transition-colors bg-violet-600 text-white hover:bg-violet-700">
                      ...
                      <div key={t.id} className="flex items-center justify-between bg-gray-50 border border-gray-100 rounded-xl px-4 py-3">
                        <div>
                          ...
                          <p className="text-[11px] text-gray-400 mt-0.5">
                            {t.gradeLevelIds.map(gid => gradeLevels.find(g => g.id === gid)?.name).filter(Boolean).join('، ') || (isRTL ? 'مش مرتبط بصف' : 'No grades linked')}
                          </p>
                        </div>
                        <button onClick={() => handleDeleteTrack(t.id)} className="text-gray-300 hover:text-red-500">
                          <X size={15} />
                        </button>
                      </div>
                    ))}
          ...
          {activeTab === SettingsTab.ACADEMIC_YEAR && (
            <div className="flex-1 flex flex-col">
              ...
              {academicYearViewMode === 'BROWSE' && schoolSettings && (
              <div className="mb-8 bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
                <h4 className="font-bold text-gray-900 mb-1">{isRTL ? 'نظام التعليم' : 'Education System'}</h4>
                <p className="text-xs text-gray-400 mb-4">{isRTL ? 'بيحدد مقياس الدرجات الافتراضي اللي بيتطبّق تلقائي على أي سجل درجات جديد في سجل الدرجات.' : 'Determines the default grading scale automatically applied to any new gradebook.'}</p>
                <div className="flex items-center gap-4">
                  <select
                    value={schoolSettings.educationSystem}
                    onChange={async (e) => {
                      const updated = { ...schoolSettings, educationSystem: e.target.value as 'british' | 'american' | 'national' };
                      setSchoolSettings(updated);
                      await updateSchoolSettings(updated);
                      showToast(isRTL ? 'تم حفظ نظام التعليم.' : 'Education system saved.', 'success');
                    }}
                    className="w-full max-w-xs p-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-violet-500"
                  >
